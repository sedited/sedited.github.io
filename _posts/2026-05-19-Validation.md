...
include all the known block headers and their tree-like data structure
(tree-like because they may include stale forks), the currently best known
chain of blocks, the UTXO set, and several caching data structures.
...
tree, a structure where each node (or rather block) has a pointer to its
parent, allowing traversal back to genesis. It may include forks, forming a
tree with a long stem and short branches. As a subcategory, they also include
the "ContextualCheck\*"-level functions. The "ContextualCheck\*" functions don't
...
serialized. Once past these checks, an entry is created for the block in the
block tree, and the block itself is written to disk. Crucially, this means a
block that passed through these checks, but is later found invalid, for example
by an invalid script in one of its transactions, is nevertheless persisted on
disk. To avoid an attacker filling the disk, single blocks below a certain
proof of work threshold are rejected.

Lastly, the block passes through the "Chain"-level validation functions. These
maintain two data structures: a vector of block tree entries from genesis to
the currently best known block for fast, height-indexed access referred to as
`CChain`, and the UTXO set (`CCoinsViewCache`), fundamentally a mapping from a
transaction out point to its corresponding transaction output (its `Coin`).
This provides both a "view" into the UTXO set persisted on disk, and a "cache"
for previously retrieved coins and coins that are spent within the same block.

If the block extends the chain, the node retrieves its coins from the
`CCoinsViewCache`. This retrieval checks that all required coins exist and that
there are no double spends. The coins are also used to check that the
transactions' input and output amounts balance. Then the script interpreter is
invoked for every input and its coin of every transaction in the block. If
these checks pass, new coins are added to the cache. All the outputs that are
spent by the block are added to a separate data structure (`CBlockUndo`) and
persisted to disk. This means that after validation, every block persisted on
disk also has a corresponding `CBlockUndo` containing all the outputs the block
has spent. This "undo" data is used during a re-org: The coins spent by blocks
no longer part of the chain are added back to the `CCoinsViewCache`. Finally,
the block is marked as valid and added to the `CChain`.
...
the UTXO set acts as a state accumulator for validation.

Performance-wise, there are two main bottlenecks: evaluating all the scripts
(including their embedded signatures) and interacting with the UTXO set. Both
sit in the "Chain"-level of validation functions, which makes these
later validation steps the most expensive. Script validation speedups mostly
come from improvements in `libsecp256k1`. A block's transactions' scripts are
also evaluated in parallel using multiple CPU cores (`CCheckQueueControl`).
Script validation is currently the only validation step happening in parallel.
Historically, around 80% of coins are spent within a day of being created
(https://pmc.ncbi.nlm.nih.gov/articles/PMC8989952/figure/Fig3/). This allows
for an efficient caching strategy: By retaining the latest coins in memory, a
cut-through optimization is achieved that avoids reading and writing the
short-lived coins to disk. Adding, reading, and removing these coins happens
entirely in memory, without any on-disk interaction.
...
Bitcoin Core also optimizes block relay and serving speed. `AcceptBlock` saves
blocks to disk in their full network serialization format. This allows a node
to respond to a `GETDATA` request without any (de-)serialization overhead. The
node also immediately relays blocks upon passing the "Accept\*"-level checks,
before the expensive script and coin validation steps. This means a node may be
relaying blocks with invalid transactions, though this is again mitigated by
already having checked that the proof of work is over a certain threshold.
...
introduces an additional trust assumption. The target block is set by the
Bitcoin Core developers at release time, but users may override it. Coin-level
checks are still run, meaning users of this feature still validate all
"Check\*"- and "Accept\*"-level checks, and ensure that no coins are
double-spent. For platforms that lack processing power, like small
system-on-a-chip computers, this means a significant reduction in
...
retrieval of coins (https://github.com/bitcoin/bitcoin/pull/35295), which
alleviates this bottleneck. This makes coin disk reads close to asynchronous,
allowing for coin cache reads to happen in the meantime. Still, no other
validation work happens in the meantime, and the node is blocked from making
progress on other tasks.

The same mutex, `cs_main`, protects the validation caches, the UTXO set, and
...
fundamental tradeoffs involved. Bitcoin Core should try to move the bottleneck
further away from the CPU and disk: If performant hardware is available, the
software should use it to the fullest extent possible.
